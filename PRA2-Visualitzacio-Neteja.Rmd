...
# Creem agrupacions per tal de poder fer histogrames

```{r}

student_global$Nota_bins <- arules::discretize(student_global$Avg_Final, method = "fixed",
...
```{r}
student_global$Avg_Final2 <- ceiling(student_global$Avg_Final)

hist1 <- student_global %>%
  group_by(sex, Avg_Final2) %>%
  summarise(total = n()) %>%
  ungroup()
  

hist2 <- student_global %>%
  group_by(Avg_Final2) %>%
  summarise(total = n()) %>%
  ungroup() %>%
  mutate(sex = "Total")

col_order = c("sex", "Avg_Final2", "total")

hist2 <- hist2[, col_order]

hist1 <- rbind(hist2, hist1)

```

```{r}
hist3 <- student_global %>%
  group_by(sex, Avg_Final2) %>%
  summarise(total = n()) %>%
  ungroup() %>%
...
addWorksheet(wb, "Hist_Genere")
writeData(wb, "Hist_Genere", hist1)

addWorksheet(wb, "Hist_Genere2")
writeData(wb, "Hist_Genere2", hist3)
...
addWorksheet(wb, "Treball_Pare")
writeData(wb, "Treball_Pare", valors_educatius3)

addWorksheet(wb, "Treball_Mare")
writeData(wb, "Treball_Mare", valors_educatius4)

addWorksheet(wb, "Consum_alcohol")
writeData(wb, "Consum_alcohol", valors_educatius5)

addWorksheet(wb, "Sortides")
writeData(wb, "Sortides", valors_educatius6)

addWorksheet(wb, "Genere")
writeData(wb, "Genere", valors_educatius7)

addWorksheet(wb, "Resultats_Exploracio")
writeData(wb, "Resultats_Exploracio", noudf)

arxiu <- "dadesNetes.xlsx"
...
```{r}
write.csv(student_mat, "mates.csv", row.names = FALSE)
write.csv(student_por, "port.csv", row.names = FALSE)
write.csv(student_global, "global.csv", row.names = FALSE)
